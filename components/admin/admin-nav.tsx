...
// styling (ui-design-system-specification.md §9).
          ...
          <nav aria-label="Admin" className="flex items-center gap-1">
            {ADMIN_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="rounded-md px-2.5 py-1.5 text-small font-medium text-text-secondary transition-colors hover:bg-surface-alt hover:text-text-primary"
              >
                {link.label}
              </Link>
            ))}
